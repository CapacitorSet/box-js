...
`
	...
	if (!argv["no-rewrite"]) {
		...
		if (argv["dumb-concat-simplify"]) {
			console.log("Applying dumb concat simplification (remove --dumb-concat-simplify to skip)...")
			code = code.replace(/'[ \r\n]*\+[ \r\n]*'/gm, "");
			code = code.replace(/"[ \r\n]*\+[ \r\n]*"/gm, "");
		}
...
`
		...
		if (!argv["no-concat-simplify"]) {
			console.log("Simplifying concatenations (use --no-concat-simplify to skip)...");
